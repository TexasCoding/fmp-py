...
@pytest.fixture
def fmp_historical_data():
    return FmpHistoricalData()


def test_fmp_historical_data_init(fmp_historical_data):
    assert isinstance(fmp_historical_data, FmpHistoricalData)


def test_fmp_historical_data_daily_history(fmp_historical_data):
    symbol = "AAPL"
    ...
    assert isinstance(data.iloc[0]["high"], np.float64)
...
def test_fmp_historical_data_daily_history_with_invalid_symbol(fmp_historical_data):
    ...
    with pytest.raises(ValueError):
        fmp_historical_data.daily_history(symbol, from_date, to_date)


def test_fmp_historical_data_intraday_history(fmp_historical_data):
    symbol = "AAPL"
    interval = "1min"
    from_date = "2023-01-01"
    to_date = "2023-01-10"
...
def test_fmp_historical_data_intraday_history_with_invalid_symbol(fmp_historical_data):
    ...
    to_date = "2023-01-10"
    with pytest.raises(ValueError):
        fmp_historical_data.intraday_history(symbol, interval, from_date, to_date)
